...
public:
    CEmulatorHost(TJBox_Float64 iSampleRate, std::vector<uint8_t> iExecutable,
                  uint32_t iOffset, uint32_t iEntry, uint32_t iMain_Stack,
                  uint32_t iEventStack);
    [[nodiscard]] static CEmulatorHost*
    CreateFromProgram(TJBox_Float64 iSampleRate, TJBox_Value iProgramString);

    CTerminal& GetTerminal() { return fTerminal; };

    void ProcessBatch(const TJBox_PropertyDiff iPropertyDiffs[],
