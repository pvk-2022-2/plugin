...
/*
        RP: The below works for __LP64__ to, although long and long long are the
   same size they are not the same type and long long is compatible with
   std::int64_t
*/
...
/**
        @brief The maximum length of Motherboard property set object names in
   Jukebox, not including the terminating zero.
*/
#    define kJBox_MaxObjectNameLen 63

/**
 @brief A value used by /transport/pattern_index to indicate that no pattern is
 selected.
 */
...
/**
        @brief Tags for accessing properties in "user_sample" Motherboard
   objects.
 */
typedef enum {
...
/**
        @brief Tags for accessing properties in "device_host" Motherboard
   objects.
 */
typedef enum {
...
typedef struct {
    ...
    TJBox_SizeT fSize;
    /** @brief The number of bytes of the BLOB that are currently loaded in
     * memory. */
...
typedef struct {
    ...
    /** @brief The number of frames of the sample that are currently loaded in
     * memory. */
...
typedef struct {
    ...
    /**
            @brief Name of the property in the object as a zero-terminated ASCII
       string.
            @details Max length of the property name is
       kJBox_MaxPropertyNameLen, not including the terminating zero.
    */
...
/**
    ...
    @details
        ...
        note events to the host.
                A note event contains information about when the event happened
   (frame index), its note number, and its velocity. Events with a zero velocity
   turns off a note. Events with a velocity greater than zero turns on a note.
*/
typedef struct {
    /** @brief The note number of the event, valid values are between 0 and 127.
     */
    TJBox_UInt8 fNoteNumber;
    /** @brief The velocity of the event, valid values are between 0 and 127. 0
     * velocity means that the note is turned off.*/
