...
public:
    ...
    CEmulatorThread(uint32_t iStackAddress)
        : fStackAddress(iStackAddress), fState(EThreadState::eError){};
    ...
    uint64_t StepN(uint64_t iStepCount, Memory& iMemory) {
        ...
        if (iStepCount > 0) {
            const bool finished = (fRegisterFile.get_pc() &
                                   kFinishedExecMask) == kFinishedExecMask;
