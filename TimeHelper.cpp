...
namespace {
    ...
    std::uint16_t PPQToFrameIndex(TJBox_Float64 iPPQ, TJBox_Float64 iTempo,
                                  TJBox_Float64 iSampleRate) {
        ...
        frameIndex = static_cast<std::uint16_t>(
            std::max<TJBox_Float64>(0, std::min<TJBox_Float64>(frames, 63)));
        JBOX_ASSERT(frameIndex >= 0 && frameIndex < kBatchSize);
        return frameIndex;
    }

    std::int64_t FindNextBeat(std::int64_t iBeatLength,
                              std::int64_t iMaxBeatPosition) {
        JBOX_ASSERT(iBeatLength > 0);
        const std::int64_t absolutePPQ =
            static_cast<std::int64_t>((iMaxBeatPosition) / iBeatLength) *
            iBeatLength;
        return absolutePPQ;
    }
...
CTimeHelper::CTimeHelper(TJBox_Float64 iSampleRate)
    : fIsPlaying(false), fSampleRate(iSampleRate), fLastPosition(0) {
...
TJBox_Float64 CTimeHelper::ComputeBatchLengthPPQ() const {
    const TJBox_Float64 batchLengthPPQ =
        (kBatchSize / fSampleRate) * ((fTempo / 60) * kPPQResolution);
    return batchLengthPPQ;
}

// Returns two ranges to support looping
std::pair<TPPQRange, TPPQRange> CTimeHelper::ComputePPQRangesForCurrentBatch(
    TJBox_Float64 iBatchLengthPPQ) const {
    TPPQRange firstRange;
    TPPQRange
        secondRange; // Only used if the main seequencer has looped this batch

    TJBox_Float64 batchBeginPPQ =
        JBox_LoadMOMPropertyAsNumber(fTransportRef, kJBox_TransportPlayPos);
    ...
    if (fIsLooping) {
        ...
        const std::int64_t loopEndPPQ =
            static_cast<std::int64_t>(JBox_LoadMOMPropertyAsNumber(
                fTransportRef, kJBox_TransportLoopEndPos));

        if (IsInRange(loopEndPPQ, firstRange)) {
            const std::int64_t loopBeginPPQ =
                static_cast<std::int64_t>(JBox_LoadMOMPropertyAsNumber(
                    fTransportRef, kJBox_TransportLoopStartPos));

            firstRange.second = loopEndPPQ;
            secondRange.first = loopBeginPPQ;
            const TJBox_Float64 secondRangeLength =
                iBatchLengthPPQ - (loopEndPPQ - batchBeginPPQ);
    ...
    JBOX_ASSERT(!IsEmptyRange(firstRange) ||
                (IsEmptyRange(firstRange) && IsEmptyRange(secondRange)));
    JBOX_ASSERT(IsValidRange(secondRange));
    return std::make_pair(firstRange, secondRange);
}

void CTimeHelper::ProcessRange(const TPPQRange& iRange,
                               CEventManager& iEventManager) {

    // Output the denominator beat if it happens in the current batch
    {
        const std::int64_t beatLength =
            kPPQResolution; // Denominator is always on quarter note beats
        ...
        if (IsInRange(nextBeat, iRange)) {
            ...
            const std::uint16_t frameIndex =
                PPQToFrameIndex(offsetPPQ, fTempo, fSampleRate);
...
void CTimeHelper::HandleBatch(CEventManager& iEventManager,
    ...
    for (TJBox_UInt32 i = 0; i < iDiffCount; ++i) {
        ...
        if (diff.fPropertyRef.fObject == fTransportRef &&
            diff.fPropertyTag == kJBox_TransportPlaying)
            fIsPlaying = JBox_GetBoolean(diff.fCurrentValue);

        else if (diff.fPropertyRef.fObject == fTransportRef &&
                 diff.fPropertyTag == kJBox_TransportLoopEnabled)
            fIsLooping = JBox_GetBoolean(diff.fCurrentValue);

        else if (diff.fPropertyRef.fObject == fTransportRef &&
                 diff.fPropertyTag == kJBox_TransportTempo)
            fTempo = JBox_GetNumber(diff.fCurrentValue);

        else if (diff.fPropertyRef.fObject == fEnvironmenRef &&
                 diff.fPropertyTag == kJBox_EnvironmentPlayerBypassed)
            fIsBypassedByHost = JBox_GetBoolean(diff.fCurrentValue);

        else if (diff.fPropertyRef.fObject == fCustomPropertiesRef &&
                 diff.fPropertyTag == kOnOffTag)
